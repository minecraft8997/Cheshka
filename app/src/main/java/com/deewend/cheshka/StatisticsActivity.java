...
public class StatisticsActivity extends CheshkaActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            diceView.setContentDescription(getString(R.string.dice_face_view_description_text, n));
            ...
            int myStat = myStats[i];
            int opponentStat = opponentStats[i];
            myCount.setText(String.valueOf(myStat));
            myOverallCount.setText("/" + myOverallStats[i]);
            opponentCount.setText(String.valueOf(opponentStat));
            opponentOverallCount.setText("/" + opponentOverallStats[i]);
